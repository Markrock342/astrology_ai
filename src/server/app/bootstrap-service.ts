...
/** Map missing user rows to session expiry (client should sign out). */
export function mapBootstrapUserError(err: unknown): never {
...
function settled<T>(
  ...
  fallback: T,
  opts?: { rethrowStaleUser?: boolean },
): T {
  if (result.status === "fulfilled") return result.value;
  const reason = result.reason;
  if (opts?.rethrowStaleUser) {
    mapBootstrapUserError(reason);
  }
...
export async function getAppBootstrap(userId: string) {
  ...
  const me = settled(meResult, null as never, { rethrowStaleUser: true });
