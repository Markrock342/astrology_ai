...
import { saveSettingDraft } from "@/server/admin/settings-admin-service";
...
/** PUT /api/admin/settings/:key/draft */
export async function PUT(req: Request, ctx: { params: Promise<{ key: string }> }) {
  return handle(async () => {
    ...
    return ok(await saveSettingDraft(key, parsed, { id: admin.id, ip }));
  });
}
