...
const GEOCODER_BASE_URL = (
  env.GEOCODER_BASE_URL ?? "https://nominatim.187.77.155.129.nip.io"
).replace(/\/+$/, "");
const GEOCODER_FALLBACK_BASE_URL = (
  env.GEOCODER_FALLBACK_BASE_URL ?? PUBLIC_NOMINATIM_URL
).replace(/\/+$/, "");

// The public fallback caps the whole application at 1 req/s. Coolify runs one
// web process, so serialize fallback cache misses here in addition to per-user
// limits. Normal production traffic goes to our Thailand instance instead.
...
async function fetchReverse(
  ...
  if (baseUrl === PUBLIC_NOMINATIM_URL) await waitForGeocoderSlot();
  return fetch(url, {
    headers: {
...
export async function POST(req: Request) {
  return handle(async () => {
    ...
    try {
      response = await fetchReverse(GEOCODER_BASE_URL, latitude, longitude);
      if (!response.ok) throw new Error(`primary geocoder: ${response.status}`);
    } catch (primaryError) {
      if (GEOCODER_FALLBACK_BASE_URL === GEOCODER_BASE_URL) throw primaryError;
      console.warn(
        "[geo] primary reverse geocoder unavailable; using policy-limited fallback",
      );
      response = await fetchReverse(
        GEOCODER_FALLBACK_BASE_URL,
        latitude,
        longitude,
      );
    }
