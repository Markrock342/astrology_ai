...
**Branch งานล่าสุด:** `be/wave-e-handoff` (รอ merge → `main`)  
**Production:** https://astrology-ai-three.vercel.app  
**รายละเอียดเต็ม:** [BE_ASSIGN.md](./BE_ASSIGN.md) · บันทึกรอบนี้: [docs/backend_wave_e.md](./docs/backend_wave_e.md) · คู่ FE: [HANDOFF_FE.md](./HANDOFF_FE.md)
...
## สถานะตอนนี้ (อัปเดต 12 ก.ค. 2026)

✅ **HANDOFF_BE ครบ 100%** บน `be/wave-e-handoff` — BE-E0.3, E1.2–E1.6  
⏳ รอ merge PR · seed `CREDIT_TOPUP` บน prod · แจ้ง FE contract  
...
- [x] เพิ่ม field บน `Payment` + migrate
- [x] เซ็ตตอน `notifyUserPaymentReviewed` สำเร็จ/ล้มเหลว
- [x] ส่ง field ใน admin payments list API (FE-E2.4 จะโชว์ badge)
...
- [x] re-run count **ใน** `$transaction` ก่อน `deductCredits`
- [x] `FOR UPDATE` บน `credit_wallets` ต้น tx
- [x] test: N concurrent, `dailyLimit=1` → SUCCESS แค่ 1
...
- [x] สร้าง `src/app/api/me/usage/route.ts`
- [x] reuse `bangkokBoundaries` + count จาก quota-service
- [x] (optional) paginated history จาก CreditTransaction / readings
...
- [x] โควต้าหมด → `QUOTA_EXCEEDED` (ไม่ใช่ `RATE_LIMITED`)
- [x] บอก FE ว่า error code ใหม่พร้อมใช้
...
- [x] product / flag “credit-only top-up”
- [x] `reviewPayment` แยก branch: top-up vs package (ไม่แตะ subscription)
- [x] ส่ง API/contract ให้ FE ทำฟอร์ม
...
- [x] `DELETE /api/me/account`
- [x] `DELETE /api/admin/users/[id]` + `requireSuperAdmin` + audit
- [x] ลบ slip blobs ที่เกี่ยวข้อง
